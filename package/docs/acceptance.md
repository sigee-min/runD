...
| Pipeline author | Declare dependent Program steps once, bind resident Buffers in declaration order, prepare once, and reuse one move-only Pipeline through standalone or Session execution. | One fluent `pipeline(device).then(program, read(...), write(...)).prepare()` path plus `tile_repeat<N>(seed, action, fold)` inside bounded resident windows; no dependency handles, graph unrolling, DAG, reorder, implicit transfer, or second Session result vocabulary. The second Program observes the first Program's resident write; nested recurrence reuses tile-local banks while preserving the outer fold; a nonempty GPU tick submits once; outputs remain unavailable until explicit typed `read`. | The installed [`pipeline.cpp`](../tests/consumer/example/pipeline.cpp), `tools/test/run compute.pipeline`, `tools/test/run compute.window`, `tools/test/run runtime.compute-pipeline`, accelerator Session coverage, installed release consumer, and Pipeline build/compute measurements. |
