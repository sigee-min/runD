...
The current package version is `1.0.0`. CMake install rules in
...
fresh external project with `find_package(runD 1.0.0 EXACT CONFIG REQUIRED)`,
...
configure accepts only the exact `1.0.0` version. All three external configure
probes name
...
```text
.cache/release/dist/rund-sdk-1.0.0-darwin-arm64.tar.gz
.cache/release/dist/rund-sdk-1.0.0-darwin-arm64.sha256
...
The 1.0.0 archive, checksum, and verifier may be externally hosted only after
