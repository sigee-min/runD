...
int test_compute_fixed_format_and_policy_change_identity() {
  const auto make = []<rund::kernel::u8 I, rund::kernel::u8 F,
    ...
    const auto body =
        rund::compute_dsl::bind(1u)
            .template fixed<I, F, Round, Overflow, Approximation>()
  ...
  const auto overflow =
      make.template
      operator()<1u, 31u, rund::kernel::ComputeRounding::NearestEven,
...
int test_compute_mul_add_fixed_uses_tight_signed_bound() {
  ...
  const auto body = rund::compute_dsl::bind(1u)
                        ...
                        .write<"output">(output);
  const auto op =
      rund::compute_dsl::def("fixed-mul-add-carry")
          .on(body)
          .map([](auto i, auto b) {
            const auto lhs_values = b.template read<"lhs">();
            const auto rhs_values = b.template read<"rhs">();
            const auto addend_values = b.template read<"addend">();
            const auto output_values = b.template write<"output">();
            output_values[i] = rund::compute_dsl::quantize<16u, 16u>(
                rund::compute_dsl::mul_add_fixed(lhs_values[i], rhs_values[i],
                                                 addend_values[i]));
          });

  TEST_ASSERT(op.ok());
  auto parsed = rund::kernel::compute_lowering_detail::ParseComputeIR(op.ir());
...
int test_compute_formatted_constant_owns_numeric_policy() {
  ...
  i64 input[1]{};
  const auto body =
      rund::compute_dsl::bind(1u).fixed<20u, 44u>().read<"input">(input);
...
int test_compute_ternary_rejects_split_precision_over_128_bits() {
  ...
  const auto select_true = FormattedConstant(select_anchor, 1u, integer_heavy);
  ...
  BuildContext clamp_context{body.bindings(), ScalarMode::FixedLane64, storage};
  const auto clamp_anchor = DynamicRead(clamp_context, 0u, storage);
  const auto clamp_value = FormattedConstant(clamp_anchor, 1u, integer_heavy);
  const auto clamp_low = FormattedConstant(clamp_anchor, 2u, integer_heavy);
  const auto clamp_high = FormattedConstant(clamp_anchor, 3u, fraction_heavy);
...
int test_compute_fixed_format_query_is_read_only_and_never_defaults() {
  ...
  using namespace rund::kernel;
  i32 fixed_input[1]{};
  const auto fixed_body =
      rund::compute_dsl::bind(1u).fixed<16u, 16u>().read<"input">(fixed_input);
...
int RunComputeDslFixedIdentityContract() {
  ...
  return test_compute_fixed_format_query_is_read_only_and_never_defaults();
}
