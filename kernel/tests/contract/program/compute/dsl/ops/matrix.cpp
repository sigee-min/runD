...
template <typename T> [[nodiscard]] auto BuildMatrixOp() {
  T m00[4]{};
  T m01[4]{};
  T m02[4]{};
  T m10[4]{};
  ...
  return rund::compute_dsl::def("dsl-matrix").on(body).map([](auto i, auto bind) {
    ...
    out[i] = rund::compute_dsl::mat(Axis::X, m00[i], m01[i], x[i], y[i]) +
             rund::compute_dsl::mat(Axis::Y, m10[i], m11[i], x[i], y[i]) +
             rund::compute_dsl::mat(MatOp::Transpose, Axis::X, m00[i], m10[i],
                              x[i], y[i]) +
             rund::compute_dsl::mat(MatOp::Transpose, Axis::Y, m01[i], m11[i],
                              x[i], y[i]) +
             rund::compute_dsl::mat(MatOp::Trace, m00[i], m11[i]) +
             rund::compute_dsl::mat(MatOp::Determinant, m00[i], m01[i], m10[i],
                              m11[i]) +
             rund::compute_dsl::mat(MatOp::Solve, Axis::X, m00[i], m01[i], m10[i],
                              m11[i], x[i], y[i]) +
             rund::compute_dsl::mat(MatOp::Solve, Axis::Y, m00[i], m01[i], m10[i],
                              m11[i], x[i], y[i]) +
             rund::compute_dsl::mat(MatOp::Determinant, m00[i], m01[i], m02[i],
                              m10[i], m11[i], m12[i], m20[i], m21[i],
                              m22[i]) +
             rund::compute_dsl::mat(MatOp::Trace, m00[i], m11[i], m22[i]) +
             rund::compute_dsl::triple(m00[i], m01[i], m02[i], m10[i], m11[i],
                                  m12[i], m20[i], m21[i], m22[i]) +
             rund::compute_dsl::mat(Axis::X, m00[i], m01[i], m02[i], x[i], y[i],
                              z[i]) +
             rund::compute_dsl::mat(Axis::Y, m10[i], m11[i], m12[i], x[i], y[i],
                              z[i]) +
             rund::compute_dsl::mat(Axis::Z, m20[i], m21[i], m22[i], x[i], y[i],
                              z[i]) +
             rund::compute_dsl::mat(MatOp::Transpose, Axis::X, m00[i], m10[i],
                              m20[i], x[i], y[i], z[i]) +
             rund::compute_dsl::mat(MatOp::Transpose, Axis::Y, m01[i], m11[i],
                              m21[i], x[i], y[i], z[i]) +
             rund::compute_dsl::mat(MatOp::Transpose, Axis::Z, m02[i], m12[i],
                              m22[i], x[i], y[i], z[i]);
  });
}
...
[[nodiscard]] bool HasMatrixOps(const std::string& source) {
...
int test_compute_matrix_helpers_lower_through_existing_ops() {
  const auto first32 = BuildMatrixOp<i32>();
  const auto second32 = BuildMatrixOp<i32>();
  const auto fixed_lane64 = BuildMatrixOp<i64>();
  ...
  TEST_ASSERT(HasMatrixOps(metal32.source_text));
  TEST_ASSERT(HasMatrixOps(vulkan64.source_text));
  return 0;
}
...
int RunComputeDslOpsMatrixContract() {
  return test_compute_matrix_helpers_lower_through_existing_ops();
}
