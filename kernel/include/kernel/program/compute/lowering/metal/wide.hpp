...
inline void AppendMetalWideFixedHelpers(std::string &out) {
  out += R"METAL(
struct RundWide { ulong lo; ulong hi; };
inline RundWide RundWideMake(ulong lo, ulong hi) { return RundWide{lo, hi}; }
inline RundWide RundWideZero() { return RundWideMake(0ul, 0ul); }
inline RundWide RundWideOne() { return RundWideMake(1ul, 0ul); }
inline bool RundWideNegative(RundWide value) {
  return (value.hi & 0x8000000000000000ul) != 0ul;
}
inline RundWide RundWideFrom32(int value) {
  const ulong sign = value < 0 ? 0xfffffffffffffffful : 0ul;
  const ulong lo = ulong(uint(value)) | (sign & 0xffffffff00000000ul);
  return RundWideMake(lo, sign);
}
inline RundWide RundWideFrom64(long value) {
  return RundWideMake(as_type<ulong>(value),
                      value < 0 ? 0xfffffffffffffffful : 0ul);
}
inline RundWide RundWideUnsignedLane(RundWide value, uint width) {
  return RundWideMake(width == 32u ? value.lo & 0xfffffffful : value.lo, 0ul);
}
inline RundWide RundWideAdd(RundWide lhs, RundWide rhs) {
  const ulong lo = lhs.lo + rhs.lo;
  return RundWideMake(lo, lhs.hi + rhs.hi + (lo < lhs.lo ? 1ul : 0ul));
}
inline RundWide RundWideSub(RundWide lhs, RundWide rhs) {
  return RundWideMake(lhs.lo - rhs.lo,
                      lhs.hi - rhs.hi - (lhs.lo < rhs.lo ? 1ul : 0ul));
}
inline RundWide RundWideNeg(RundWide value) {
  return RundWideAdd(RundWideMake(~value.lo, ~value.hi), RundWideOne());
}
inline RundWide RundWideAbs(RundWide value) {
  return RundWideNegative(value) ? RundWideNeg(value) : value;
}
inline bool RundWideUnsignedLess(RundWide lhs, RundWide rhs) {
  return lhs.hi < rhs.hi || (lhs.hi == rhs.hi && lhs.lo < rhs.lo);
}
inline bool RundWideEqual(RundWide lhs, RundWide rhs) {
  return lhs.hi == rhs.hi && lhs.lo == rhs.lo;
}
inline bool RundWideSignedLess(RundWide lhs, RundWide rhs) {
...
inline RundWide RundWideShrUnsigned(RundWide value, uint amount) {
  if (amount == 0u) return value;
  if (amount < 64u) {
...
inline RundWide RundWideShrSigned(RundWide value, uint amount) {
  if (!RundWideNegative(value)) return RundWideShrUnsigned(value, amount);
  const RundWide inverted = RundWideMake(~value.lo, ~value.hi);
  const RundWide shifted = RundWideShrUnsigned(inverted, amount);
  return RundWideMake(~shifted.lo, ~shifted.hi);
}
inline RundWide RundWideAnd(RundWide lhs, RundWide rhs) {
  return RundWideMake(lhs.lo & rhs.lo, lhs.hi & rhs.hi);
}
inline RundWide RundWideOr(RundWide lhs, RundWide rhs) {
  return RundWideMake(lhs.lo | rhs.lo, lhs.hi | rhs.hi);
}
inline RundWide RundWideXor(RundWide lhs, RundWide rhs) {
  return RundWideMake(lhs.lo ^ rhs.lo, lhs.hi ^ rhs.hi);
}
inline RundWide RundWideNot(RundWide value) {
  return RundWideMake(~value.lo, ~value.hi);
}
inline RundWide RundWideMul64(ulong lhs, ulong rhs) {
  const ulong mask = 0xfffffffful;
  const ulong lhs_lo = lhs & mask;
  const ulong lhs_hi = lhs >> 32u;
  const ulong rhs_lo = rhs & mask;
  const ulong rhs_hi = rhs >> 32u;
  const ulong p0 = lhs_lo * rhs_lo;
  const ulong p1 = lhs_lo * rhs_hi;
  const ulong p2 = lhs_hi * rhs_lo;
  const ulong p3 = lhs_hi * rhs_hi;
  const ulong carry = (p0 >> 32u) + (p1 & mask) + (p2 & mask);
  return RundWideMake((p0 & mask) | (carry << 32u),
                      p3 + (p1 >> 32u) + (p2 >> 32u) + (carry >> 32u));
}
inline RundWide RundWideMul(RundWide lhs, RundWide rhs) {
  const RundWide base = RundWideMul64(lhs.lo, rhs.lo);
  return RundWideMake(base.lo,
                      base.hi + lhs.lo * rhs.hi + lhs.hi * rhs.lo);
}
inline RundWide RundWideQuantize(RundWide value, uint source_fraction,
                                 uint target_fraction, uint rounding,
                                 uint overflow, uint width);
inline RundWide RundWideQuantizeUnsignedFixedProduct(
  ...
  return width == 32u ? RundWideFrom32(int(uint(quotient.lo)))
                      : RundWideFrom64(as_type<long>(quotient.lo));
}
inline RundWide RundWideUnsignedDiv(RundWide numerator,
                                    RundWide denominator) {
  ...
  for (int step = 0; step < 128; ++step) {
    ...
    const ulong incoming = bit < 64u ? ((numerator.lo >> bit) & 1ul)
...
inline RundWide RundWideSqrtFixed(RundWide value, uint fraction,
  ...
  for (int step = 0; step < 64; ++step) {
    ...
    const RundWide candidate = RundWideOr(root, RundWideShl(RundWideOne(), bit));
...
inline RundWide RundWideBool(bool value) {
  return value ? RundWideOne() : RundWideZero();
}
inline bool RundWideTruthy(RundWide value) { return value.lo != 0ul || value.hi != 0ul; }
inline RundWide RundWideWrap(RundWide value, uint width) {
  if (width == 32u) return RundWideFrom32(int(uint(value.lo)));
  return RundWideFrom64(as_type<long>(value.lo));
}
inline RundWide RundWideQuantize(RundWide value, uint source_fraction,
                                 uint target_fraction, uint rounding,
                                 uint overflow, uint width) {
  const RundWide low = width == 32u
      ? RundWideFrom32(int(uint(0x80000000u)))
      : RundWideFrom64(as_type<long>(0x8000000000000000ul));
  const RundWide high = width == 32u
      ? RundWideFrom32(int(uint(0x7fffffffu)))
      : RundWideFrom64(as_type<long>(0x7ffffffffffffffful));
  ...
  if (target_fraction > source_fraction) {
    ...
    const bool nonzero = !RundWideEqual(remainder, RundWideZero());
    const RundWide halfway = RundWideShl(RundWideOne(), shift - 1u);
    const bool above_half = RundWideUnsignedLess(halfway, remainder);
    const bool at_half = RundWideEqual(remainder, halfway);
    const bool nearest = above_half || (at_half && (quotient.lo & 1ul) != 0ul);
    if ((rounding == 2u && negative && nonzero) ||
        (rounding == 3u && !negative && nonzero) ||
        (rounding == 4u && nearest)) {
      quotient = RundWideAdd(quotient, RundWideOne());
