...
prepare_backend_stream(PipelineState &state, const bool alternate,
                       Location &location) {
  // Pipeline step capacity is a public admission bound. Keep the cold
  // projection in fixed storage so primary/alternate preparation cannot create
  // an unplanned vector layer before the backend budget gate.
  ...
  std::array<node::accel::detail::BackendRecurrence, capacity> recurrences{};
  ...
  for (std::size_t index = 0u; index < state.steps.size(); ++index) {
    ...
    const std::shared_ptr<JobState> &job =
        alternate ? state.steps[index].alternate_job : state.steps[index].job;
    if (job == nullptr) {
      return Result<node::accel::detail::PreparedKernelPipeline>::fail(
      ...
      if (has_terminal) {
        ...
        for (std::uint32_t bank = PipelineWindow::seed;
          ...
          if (!resolve_view(state, planned->sources[bank], alternate,
                            terminal[bank]) ||
              terminal[bank].source.element_bytes != sizeof(std::uint32_t) ||
  ...
  for (std::size_t index = 0u; index < state.publications.size(); ++index) {
    ...
    const PipelineWindow &publication_window = state.windows[publication_state];
    ...
    auto identity = project_pipeline_publication_identity(
        publication, control,
        window == nullptr ? 0u : publication_window.nested_shape.outer_bound());
