...
struct MapRecipe final {
  ...
  [[nodiscard]] bool active() const noexcept { return !outputs.empty(); }
};

[[nodiscard]] bool project_expressions(
    std::span<const ExprRef> expressions, const StepLivePlan &map_plan,
...
plan_maps(const FlowState &flow, const std::vector<bool> &keep,
          ...
          std::span<const std::size_t> order, std::vector<MapRecipe> &recipes,
          std::vector<MapRecipe> &baseline, std::vector<std::uint8_t> &skipped);
