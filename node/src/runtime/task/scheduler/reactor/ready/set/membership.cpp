...
::rund::net::ready::Status Scheduler::AddReadyInterest(
  ...
  const ReactorInterest interest =
      ::rund::net::ReactorInterestFor(request.interest);
  if (interest == ReactorInterest::None) {
    return finish(ReasonCode::TaskInvalid);
  }
  if (ReactorReadySetHasDuplicate(*set, request.socket, interest)) {
    return finish(ReasonCode::TaskInvalid);
  }
  ...
  return finish(ReasonCode::Ok, set->identity.handle);
}

::rund::net::ready::Status Scheduler::RemoveReadyInterest(
    const ::rund::net::ready::Set handle,
    const ::rund::net::ready::Request request) noexcept {
  (void)TrapLaneOwnedSegmentPrimitive();
  EnsureCurrentCommit();
  ...
  for (auto member = set->members.begin(); member != set->members.end();
       ++member) {
    if (member->socket == request.socket && member->interest == interest) {
