...
namespace {
constexpr std::size_t MaxReusableMetalBuffers = 32u;
}

void ReleaseMetalBuffer(MetalAdapter &adapter, MetalRuntimeBuffer buffer) {
  if (buffer.borrowed || buffer.buffer == nullptr || buffer.bytes == 0u) {
    return;
  }
  ...
  if (adapter.free_buffers.size() < MaxReusableMetalBuffers) {
