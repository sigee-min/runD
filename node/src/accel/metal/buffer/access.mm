...
#if defined(__APPLE__) && defined(RUND_NODE_HAVE_METAL_SDK)
bool UploadMetalBuffer(MetalAdapter &adapter, const MetalRuntimeBuffer &buffer,
                       const void *const data, const rund::kernel::u64 bytes) {
  if (bytes == 0u) {
    return true;
  }
...
#else
bool UploadMetalBuffer(MetalAdapter &, const MetalRuntimeBuffer &, const void *,
                       rund::kernel::u64) {
  return false;
}

void *MetalBufferContents(const MetalRuntimeBuffer &) { return nullptr; }
#endif
