...
namespace {
[[nodiscard]] const char *MetalScanBlockU32Source() {
  return R"MSL(
kernel void rund_compute_scan_block_u32(
  ...
  threadgroup uint values[2][kScanWidth];
  ...
  values[0][tid] = running;
  uint offset = 0u;
  uint total = 0u;
  rund_scan_exclusive_uint_pair(values[0], values[1], tid, width, offset,
                                total);
  running = offset;
  uint bad = 0u;
  for (ulong index = lane_begin; index < lane_end; ++index) {
    const uint value = input[index];
    const uint next = running + value;
    output[index] = canonical_here != 0u && inclusive == 0u ? running : next;
    if (canonical_here != 0u &&
        rund_scan_overflow_uint(running, value, next, signed_domain)) {
      bad = 1u;
    }
    running = next;
  }
