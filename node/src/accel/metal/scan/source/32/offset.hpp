...
[[nodiscard]] const char *MetalScanOffsetU32Source() {
  return R"MSL(
kernel void rund_compute_scan_offset_u32(
    ...
    device const uint* input [[buffer(8)]],
  ...
  const uint offset = offsets[block];
  uint bad = 0u;
  for (ulong index = lane_begin; index < lane_end; ++index) {
    const uint local = output[index];
    const uint value = input[index];
    const uint global = offset + local;
    const uint previous = global - value;
    if (rund_scan_overflow_uint(previous, value, global, signed_domain)) {
      bad = 1u;
    }
