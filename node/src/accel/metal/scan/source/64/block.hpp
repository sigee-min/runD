...
namespace {
[[nodiscard]] const char *MetalScanBlockU64Source() {
  return R"MSL(
kernel void rund_compute_scan_block_u64(
    device const ulong* input [[buffer(0)]],
    device ulong* output [[buffer(1)]],
    device ulong* totals [[buffer(2)]],
  ...
  for (ulong index = lane_begin; index < lane_end; ++index) {
    ...
    const ulong next = running + value;
    output[index] = canonical_here != 0u && inclusive == 0u ? running : next;
    if (canonical_here != 0u &&
        rund_scan_overflow_ulong(running, value, next, signed_domain)) {
      bad = 1u;
    }
    running = next;
  }
