...
namespace {
std::shared_ptr<void> FindLocked(MetalAdapter &adapter,
  ...
  for (auto entry = begin; entry != end; ++entry) {
    ...
    if (cached.source_hash == source_hash && cached.key == artifact.key &&
        cached.source == artifact.source_text && cached.pipeline != nullptr) {
...
FindMetalMapArtifactPipeline(MetalAdapter &adapter,
  ...
  return FindLocked(adapter, artifact, source_hash);
}
...
StoreMetalMapArtifactPipeline(MetalAdapter &adapter,
  ...
  const std::uint64_t source_hash = SourceHash(artifact.source_text);
  if (auto cached = FindLocked(adapter, artifact, source_hash)) {
    return cached;
  }
