...
[[nodiscard]] MetalPipelineSourceRecipe
MetalHistogramPipelineSourceRecipe(const std::uint64_t bins) noexcept {
  return MetalSourceRecipe(0x6d6574616c686973ull ^
                               (MetalHistogramLocal(bins) ? 0u : 1u),
                           MetalHistogramSourceUpperBytes(bins), 2u, 2u);
}
  ...
  case rund::kernel::NodeKind::Histogram:
    ...
    if (!AddMetalPipelineSourceRecipe(
            ...
            MetalHistogramPipelineSourceRecipe(
                step.operation.get<operation::Histogram>().plan.bin_count))) {
      return false;
    }
    break;
