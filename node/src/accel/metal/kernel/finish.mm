...
rund::AccelCheck FinishMetalStep(MetalAdapter &adapter,
...
rund::AccelCheck FinishMetalSteps(MetalAdapter &adapter,
  ...
  const rund::AccelCheck result = finish::Steps(
      resources,
      [&](const MetalKernelEntry &entry) {
        return FinishMetalStep(adapter, entry.ops, entry.resource);
      },
      &failed);
  if (!result.ok && stats != nullptr && failed < resources.size()) {
    const MetalKernelEntry *const entry = resources.entry(failed);
