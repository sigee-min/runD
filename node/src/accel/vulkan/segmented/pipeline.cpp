...
PseudoSegmentedScanPlan(const rund::kernel::SegmentedScanDesc &desc,
  ...
  const rund::kernel::SegmentedScanHash hash = rund::kernel::HashSegmentedScan(
      rund::kernel::SegmentedScanDesc{.op = desc.op, .element = desc.element});
...
AcquireSegmentedScanPipeline(VulkanAdapter &adapter,
                             const rund::kernel::SegmentedScanDesc &desc,
                             const rund::kernel::ComputeDomain domain,
                             const VulkanSegmentedScanStage stage) {
  const rund::kernel::ComputePlan pseudo = PseudoSegmentedScanPlan(
      desc, domain, rund::kernel::ComputeApi::Vulkan, stage);
  ...
  const std::uint32_t push_bytes =
      stage == VulkanSegmentedScanStage::Prefix ? 0u : kSegmentedScanPushBytes;
