...
void EncodeVulkanCompactClassify(const VulkanCompactEncodeResources &compact,
                                 const VkCommandBuffer command,
                                 const std::uint32_t workgroups) {
  BindVulkanPipeline(command, VK_PIPELINE_BIND_POINT_COMPUTE,
                     compact.classify_pipeline->pipeline);
  BindVulkanDescriptors(command, VK_PIPELINE_BIND_POINT_COMPUTE,
                        compact.classify_pipeline->pipeline_layout, 0u, 1u,
                        &compact.classify_set, 0u, nullptr);
  DispatchVulkan(command, workgroups, 1u, 1u);
}

void EncodeVulkanCompactCountsBarrier(
    const VulkanCompactEncodeResources &compact,
    const VkCommandBuffer command) {
  const VkBufferMemoryBarrier barrier = VulkanBufferBarrier(
      compact.counts, VK_ACCESS_SHADER_WRITE_BIT, VK_ACCESS_SHADER_READ_BIT);
