...
bool ExecuteWindow(VulkanAdapter &adapter, VulkanCachedPipeline &pipeline,
  ...
  if (!PrepareVulkanWindowBuffers(adapter, plan, window, bindings, input_plans,
                                  resident_bindings, buffers)) {
    return false;
  }
  VkDescriptorSet descriptor_set = VK_NULL_HANDLE;
  if (!UpdateVulkanWindowDescriptorSet(
          adapter, pipeline, plan, window, bindings, input_plans, param_buffer,
          resident_bindings, buffers, descriptor_set)) {
    return false;
  }
