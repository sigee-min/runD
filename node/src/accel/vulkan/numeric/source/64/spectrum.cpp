...
[[nodiscard]] bool EmitSpectrumSource64(Sink &sink)
...
[[nodiscard]] std::string SpectrumSource64() {
  ...
  const auto emit = [](auto &sink)
      noexcept(noexcept(EmitSpectrumSource64(sink))) {
    return EmitSpectrumSource64(sink);
  };
...
bool SpectrumSource64Bytes(std::uint64_t &bytes) noexcept {
