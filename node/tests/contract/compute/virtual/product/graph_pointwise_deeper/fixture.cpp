...
Preparation prepare_case(const rund::compute::Device &device) {
  ...
  if (!program || !validate_program(*program)) {
    std::fprintf(stderr, "Graph deeper compile reason=%u\n",
  ...
  for (std::size_t input = 0u; input < InputCount; ++input) {
    ...
    for (std::size_t index = 0u; index < ElementCount; ++index) {
      inputs[input][index] = 7u + input * 23u + index * (input + 3u);
      expected[index] +=
  ...
  auto prepared = virtual_pipeline(*program, *a, *b, *c, *d, *e, *output,
                                   ResidencyConfig{});
  if (!prepared) {
    std::fprintf(stderr, "Graph deeper prepare reason=%u\n",
