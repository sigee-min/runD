...
build_program(const rund::compute::Device &device) {
  return rund::compute::on(device)
      ...
      .zip_input<std::uint64_t>(FrameElements)
      .zip_input<std::uint64_t>(FrameElements)
      .map("virtual-graph-pointwise-wide-input",
           [](auto a, auto b, auto c, auto d, auto e, auto f, auto g) {
             return add_stage<1u, InputLeafCount>(a) +
                    ...
                    add_stage<6u * InputLeafCount + 1u, InputLeafCount>(g);
           })
      .map("virtual-graph-pointwise-wide-output",
...
bool validate_program(const Program &program) noexcept {
  ...
  const bool valid = slices && slices->stages.size() == StageCount &&
                     ...
                     slices->stages.front().inputs == slices->input_resources &&
                     slices->stages.back().inputs.size() == 1u && !fused &&
                     fused.reason() == Reason::ExpressionCapacity;
  if (!valid) {
    std::fprintf(
        stderr,
        "Graph wide program slices=%u/%u stages=%zu resources=%zu inputs=%zu "
        "first=%zu last=%zu fused=%u/%u\n",
        ...
        slices && !slices->stages.empty() ? slices->stages.front().inputs.size()
                                          : 0u,
        slices && !slices->stages.empty() ? slices->stages.back().inputs.size()
                                          : 0u,
