...
build_program(const rund::compute::Device &device) {
  return rund::compute::on(device)
      ...
      .zip_input<std::uint64_t>(FrameElements)
      .zip_input<std::uint64_t>(FrameElements)
      .map("virtual-graph-pointwise-frontier-input",
      ...
      .map("virtual-graph-pointwise-frontier-middle-a",
           [](auto value) {
             return add_stage<1u, StageLeafCount>(value) ^ 0x55ull;
           })
      .map("virtual-graph-pointwise-frontier-middle-b",
           [](auto value) {
             return add_stage<StageLeafCount + 1u, StageLeafCount>(value) *
                    3ull;
           })
      .map(
          "virtual-graph-pointwise-frontier-middle-c",
          [](auto value) {
      ...
      .map("virtual-graph-pointwise-frontier-output",
           [](auto value) {
...
bool validate_program(const Program &program) noexcept {
  ...
  if (!valid) {
    std::fprintf(stderr,
                 "Graph frontier program slices=%u/%u stages=%zu "
