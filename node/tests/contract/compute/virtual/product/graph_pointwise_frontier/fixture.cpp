...
namespace rund_node_test_virtual::product::graph_pointwise_frontier {
namespace {
...
Preparation prepare_case(const rund::compute::Device &device) {
  ...
  if (!program || !validate_program(*program)) {
    std::fprintf(stderr, "Graph frontier compile reason=%u\n",
  ...
  for (std::size_t input = 0u; input < InputCount; ++input) {
    ...
    for (std::size_t index = 0u; index < ElementCount; ++index) {
      inputs[input][index] = 11u + input * 29u + index * (input + 5u);
      expected[index] +=
  ...
  for (std::uint64_t &value : expected) {
    value = stage_value(value, 1u) ^ 0x55ull;
    value = stage_value(value, StageLeafCount + 1u) * 3ull;
    value =
        (stage_value(value, 2u * StageLeafCount + 1u) ^ 0xa5ull) * 5ull + 7ull;
    value = stage_value(value, 3u * StageLeafCount + 1u) | 0x11ull;
  }
  auto output_backing =
      std::make_shared<MemoryVirtualBacking>(LogicalBytes, PageBytes);
  ...
  auto prepared =
      virtual_pipeline(*program, *a, *b, *c, *d, *output, ResidencyConfig{});
  if (!prepared) {
    std::fprintf(stderr, "Graph frontier prepare reason=%u\n",
