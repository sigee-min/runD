...
inline constexpr std::size_t InputCount = 4u;
inline constexpr std::size_t StageCount = 5u;
...
// Four external resources, four intermediate resources, and one output fill
// the fixed nine-resource Graph boundary. Each authored stage remains below
// ExpressionCapacity, while every adjacent composition exceeds it.
inline constexpr std::size_t StageLeafCount = 240u;
