...
#include "graph_pointwise_deeper/internal.hpp"
...
int CheckProductGraphPointwiseDeeperDevice(
  ...
  auto prepared = graph_pointwise_deeper::prepare_case(*opened);
  if (!prepared.value) {
  ...
  const auto initial_generations =
      graph_pointwise_deeper::stage_generations(test_case);
  const Status status = test_case.pipeline.run();
  return graph_pointwise_deeper::validate_case(
