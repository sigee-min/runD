...
#include "graph_pointwise_deep/internal.hpp"
...
int CheckProductGraphPointwiseDeepDevice(const rund::compute::Backend backend) {
  ...
  auto prepared = graph_pointwise_deep::prepare_case(*opened);
  if (!prepared.value) {
  ...
  const auto initial_generations =
      graph_pointwise_deep::stage_generations(test_case);
  const Status status = test_case.pipeline.run();
  return graph_pointwise_deep::validate_case(
