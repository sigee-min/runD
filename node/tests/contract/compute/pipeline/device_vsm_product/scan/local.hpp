...
inline constexpr std::uint64_t TypedMask = 0x55ull;
inline constexpr std::uint64_t TypedFactor = 3ull;
inline constexpr std::uint64_t TypedAdd = 7ull;

enum class ScanMapCase : std::uint8_t {
  None,
  Immediate,
  Canonical,
  Branch,
...
struct PreparedScan final {
  ...
  ScanMapCase map{ScanMapCase::None};
...
[[nodiscard]] bool PrepareScanProduct(rund::compute::Backend, std::uint64_t,
                                      rund::kernel::ScanOp, bool, ScanMapCase,
                                      PreparedScan &, bool &);
