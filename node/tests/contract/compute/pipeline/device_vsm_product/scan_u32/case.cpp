...
[[nodiscard]] bool RunSuccess(
  ...
  if (!PrepareScanProduct(backend, pages, operation, false, prepared,
                          unavailable)) {
    return unavailable;
  }
  ...
  const bool valid =
      ...
      owner->proof->scan.semantic.element == rund::kernel::ScanElement::U32 &&
  ...
  if (!valid) {
    std::fprintf(
        stderr,
        "DeviceVsm U32 Scan success backend=%u Q=%llu op=%u "
        "status=%u route=%u/%u/%u queue=%llu/%llu output=%u "
        "reason=%s\n",
...
bool RunScanProductCases(
  ...
  for (const std::uint64_t pages : {5u, 9u, 257u}) {
    ...
    if (!RunSuccess(backend, queue_counter, pages,
                    rund::kernel::ScanOp::InclusiveSum, unavailable,
                    inclusive_retained[index]) ||
        !RunSuccess(backend, queue_counter, pages,
                    rund::kernel::ScanOp::ExclusiveSum, unavailable,
                    exclusive_retained[index])) {
      return unavailable;
    }
    if (unavailable) {
      return true;
