...
[[nodiscard]] int CheckProfile(rund::compute::Device &device,
  ...
  const std::uint64_t warm_allocation_count =
      backend == Backend::Cpu ? node_compute_allocation::Count() : 0u;
  node_compute_allocation::Start();
  const auto aggregate_profile = profiled->profile();
  node_compute_allocation::Stop();
  const std::uint64_t aggregate_allocation_count =
  ...
  const bool aggregate_stats =
      warm_profile && aggregate_profile &&
      std::memcmp(&warm_profile->execution, &aggregate_profile->execution(),
                  sizeof(Stats)) == 0;
  const bool aggregate_memory =
      warm_profile && aggregate_profile &&
      SameMemory(warm_profile->memory, aggregate_profile->memory());
  const bool aggregate_same_epoch =
      warm_profile && aggregate_profile &&
      aggregate_profile->device().backend == backend &&
      aggregate_profile->execution().backend == backend &&
      aggregate_profile->memory().backend == backend &&
      aggregate_profile->memory().scope == MemoryScope::Pipeline &&
      aggregate_stats && aggregate_memory;
  if (!warm_run || !warm_profile || !aggregate_profile ||
      warm_allocation_count != 0u || aggregate_allocation_count != 0u ||
      !retained_memory_stable || !warm_stats_stable || !warm_counters_clean ||
      !aggregate_same_epoch) {
    std::fprintf(
        stderr,
        "pipeline profile warm backend=%u run=%u profile=%u warm_alloc=%llu "
        "aggregate_alloc=%llu "
        "memory=%u stats=%u counters=%u aggregate=%u aggregate_stats=%u "
        "aggregate_memory=%u device=%u execution=%u memory_backend=%u "
        "scope=%u\n",
        static_cast<unsigned>(backend), static_cast<unsigned>(warm_run.ok()),
        static_cast<unsigned>(warm_profile.ok()),
        static_cast<unsigned long long>(warm_allocation_count),
        static_cast<unsigned long long>(aggregate_allocation_count),
        ...
        static_cast<unsigned>(aggregate_same_epoch),
        static_cast<unsigned>(aggregate_stats),
        static_cast<unsigned>(aggregate_memory),
        aggregate_profile
            ? static_cast<unsigned>(aggregate_profile->device().backend)
            : 0u,
        aggregate_profile
            ? static_cast<unsigned>(aggregate_profile->execution().backend)
            : 0u,
        aggregate_profile
            ? static_cast<unsigned>(aggregate_profile->memory().backend)
            : 0u,
        aggregate_profile
            ? static_cast<unsigned>(aggregate_profile->memory().scope)
            : 0u);
    return 8;
  }
