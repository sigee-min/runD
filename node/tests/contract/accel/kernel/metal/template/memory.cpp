...
#include "src/accel/metal/kernel/template/memory.hpp"
...
[[nodiscard]] bool ExactMetalPipelineSourceContract() {
  ...
  const std::size_t basic_begin =
      telemetry.find("kernel void rund_pipeline_telemetry_accumulate(");
  const std::size_t profiled_begin = telemetry.find(
      "kernel void rund_pipeline_telemetry_accumulate_profiled(");
  const std::size_t basic_end =
      telemetry.find("kernel void rund_pipeline_telemetry_accumulate_profiled(",
                     basic_begin);
  if (basic_begin == std::string_view::npos ||
      profiled_begin == std::string_view::npos ||
      basic_end == std::string_view::npos || basic_end <= basic_begin) {
    return false;
  }
  const std::string_view basic =
      telemetry.substr(basic_begin, basic_end - basic_begin);
  const std::string_view projection_call =
      "telemetry_step_control(primary, count_words, predicate_words, params)";
  const std::string_view merge_call =
      "merge_pipeline_telemetry(control, value);";
  return status.size() == 29009u &&
         SourceHash(status) == 0x305873dc4ce58800ull &&
         telemetry.size() == 7610u &&
         SourceHash(telemetry) == 0x5842da8cdac65eb4ull &&
         combined.size() == 36619u &&
         SourceHash(combined) == 0xa9e0757a1004ad49ull &&
         combined.substr(0u, status.size()) == status &&
         combined.substr(status.size()) == telemetry &&
         basic.find("if (params.kind == 2u)") == std::string_view::npos &&
         basic.find("control->generated_item_count") ==
             std::string_view::npos &&
         basic.find(projection_call) != std::string_view::npos &&
         basic.find(merge_call) != std::string_view::npos &&
         telemetry.find(projection_call) != std::string_view::npos &&
         telemetry.find(projection_call,
                        telemetry.find(projection_call) +
                            projection_call.size()) != std::string_view::npos &&
         telemetry.find(merge_call) != std::string_view::npos &&
         telemetry.find(merge_call,
                        telemetry.find(merge_call) + merge_call.size()) !=
             std::string_view::npos;
}
...
[[nodiscard]] bool MetalTemplateMemoryContract() {
  ...
  // Ordinary native owners must not reserve the global spatial-proof table.
  // Keep this independent of the configured maximum Pipeline step count.
  static_assert(sizeof(MetalSequence) < 64u * 1024u);
  const MetalSpatialWindowProof ordinary =
      ProveMetalSpatialWindow({}, {}, {}, {}, {}, 1u);
  if (ordinary.admitted || !ordinary.locals.empty() ||
      ordinary.locals.capacity() != 0u) {
    return false;
  }
  PreparedKernelPipelineReservation overflow{};
  overflow.backend_step_occurrence_count =
      std::numeric_limits<std::uint64_t>::max();
  const rund::AccelCheck overflow_result =
      PlanMetalPipelineStructureForCalibration(Calibration(4096u), overflow);
  if (overflow_result.ok || overflow_result.reason == nullptr ||
      std::string_view{overflow_result.reason} != "compute_pipeline_capacity") {
    return false;
  }
