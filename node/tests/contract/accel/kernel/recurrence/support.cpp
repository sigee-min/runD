...
Fixture::Fixture(const ComputeApi api, const ComputeScalar scalar,
                 const bool uniform_invariant, const bool writes_history)
    : owners{std::make_shared<int>(1), std::make_shared<int>(2),
             std::make_shared<int>(3), std::make_shared<int>(4)},
      occurrences{}, entries{}, barriers{0u, 1u} {
  ...
  for (std::size_t iteration = 0u; iteration < occurrences.size();
    ...
    item.prepared = std::make_shared<int>(1);
