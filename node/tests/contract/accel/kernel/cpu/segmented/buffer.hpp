...
BuildBuffers(const rund::AccelDevice &pick,
  ...
  out.read = rund::node::accel::CreateAccelBuffer(
      out.context,
      rund::AccelBufferDesc{.scalar_width_bytes = sizeof(rund::kernel::u32),
                            .count = kCount,
                            .usage = rund::BufferUsage::ReadOnly,
});
  out.heads = rund::node::accel::CreateAccelBuffer(
      out.context,
      rund::AccelBufferDesc{.scalar_width_bytes = sizeof(rund::kernel::u32),
                            .count = kCount,
                            .usage = rund::BufferUsage::ReadOnly,
  ...
  out.write = rund::node::accel::CreateAccelBuffer(
      out.context,
      rund::AccelBufferDesc{.scalar_width_bytes = sizeof(rund::kernel::i32),
                            .count = kCount,
                            .usage = rund::BufferUsage::WriteOnly,
});
  if (!out.context.check.ok || !op.ok() || !out.read.check.ok ||
      !out.heads.check.ok || !out.mid.check.ok || !out.write.check.ok ||
      !rund::node::accel::UploadAccelBuffer(
           out.context, out.read, work.input.data(),
           work.input.size() * sizeof(work.input[0]))
           .ok ||
      !rund::node::accel::UploadAccelBuffer(
           out.context, out.heads, work.heads.data(),
           work.heads.size() * sizeof(work.heads[0]))
           .ok) {
    return out;
  }
  return out;
