...
template <typename T>
[[nodiscard]] bool RunSqOp(std::array<T, 4u> &ax, std::array<T, 4u> &ay,
  ...
  const rund::compute_dsl::ComputeOp op =
      rund::compute_dsl::def("node-cpu-simd-sq")
          .on(body)
          .map([](auto i, auto b) {
            ...
            out[i] =
                rund::compute_dsl::len(rund::compute_dsl::MetricOp::Squared,
                                       ax[i], ay[i]) +
                rund::compute_dsl::len(rund::compute_dsl::MetricOp::Squared,
                                       ax[i], ay[i], az[i]) +
                rund::compute_dsl::dist(rund::compute_dsl::MetricOp::Squared,
                                        ax[i], ay[i], bx[i], by[i]) +
                rund::compute_dsl::dist(rund::compute_dsl::MetricOp::Squared,
                                        ax[i], ay[i], az[i], bx[i], by[i],
                                        bz[i]);
          });
...
Expected32(const rund::kernel::i32 ax, const rund::kernel::i32 ay,
  ...
  std::int64_t result = cpu::vec::Dot2_32(ax, ay, ax, ay);
  result += cpu::vec::Dot3_32(ax, ay, az, ax, ay, az);
  result += cpu::vec::Dot2_32(rund::math32::detail::ScalarSubSat(ax, bx),
                              rund::math32::detail::ScalarSubSat(ay, by),
                              rund::math32::detail::ScalarSubSat(ax, bx),
                              rund::math32::detail::ScalarSubSat(ay, by));
  result += cpu::vec::Dot3_32(rund::math32::detail::ScalarSubSat(ax, bx),
                              rund::math32::detail::ScalarSubSat(ay, by),
...
Expected64(const rund::kernel::i64 ax, const rund::kernel::i64 ay,
  ...
  rund::kernel::i128 result = cpu::vec::Dot2_64(ax, ay, ax, ay);
  result += cpu::vec::Dot3_64(ax, ay, az, ax, ay, az);
  result += cpu::vec::Dot2_64(rund::math64::detail::ScalarSubSat(ax, bx),
                              rund::math64::detail::ScalarSubSat(ay, by),
                              rund::math64::detail::ScalarSubSat(ax, bx),
                              rund::math64::detail::ScalarSubSat(ay, by));
  result += cpu::vec::Dot3_64(rund::math64::detail::ScalarSubSat(ax, bx),
                              rund::math64::detail::ScalarSubSat(ay, by),
...
[[nodiscard]] bool RunsDeterministicSqDslOps() {
  ...
  TEST_ASSERT(RunSqOp(ax32, ay32, az32, bx32, by32, bz32, out32));
  ...
  TEST_ASSERT(RunSqOp(ax64, ay64, az64, bx64, by64, bz64, out64));
