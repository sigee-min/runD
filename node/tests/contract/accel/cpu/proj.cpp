...
template <typename T>
[[nodiscard]] bool RunProjOp(std::array<T, 4u> &ax, std::array<T, 4u> &ay,
  ...
  const rund::compute_dsl::ComputeOp op =
      rund::compute_dsl::def("node-cpu-simd-proj")
          .on(body)
          .map([](auto i, auto b) {
            ...
            out[i] =
                rund::compute_dsl::unit(rund::compute_dsl::Axis::X, ax[i],
                                        ay[i]) +
                rund::compute_dsl::unit(rund::compute_dsl::Axis::Y, ax[i],
                                        ay[i]) +
                rund::compute_dsl::unit(rund::compute_dsl::Axis::X, ax[i],
                                        ay[i], az[i]) +
                rund::compute_dsl::unit(rund::compute_dsl::Axis::Y, ax[i],
                                        ay[i], az[i]) +
                rund::compute_dsl::unit(rund::compute_dsl::Axis::Z, ax[i],
                                        ay[i], az[i]) +
                rund::compute_dsl::proj(rund::compute_dsl::Axis::X, ax[i],
                                        ay[i], bx[i], by[i]) +
                ...
                rund::compute_dsl::proj(rund::compute_dsl::Axis::Y, ax[i],
                                        ay[i], az[i], bx[i], by[i], bz[i]) +
                rund::compute_dsl::proj(rund::compute_dsl::Axis::Z, ax[i],
                                        ay[i], az[i], bx[i], by[i], bz[i]);
          });
  ...
  const rund::kernel::BindingSet bindings =
      op.bindings<T>(43u, lanes, rund::kernel::ComputeApi::Metal);
...
Expected32(const rund::kernel::i32 ax, const rund::kernel::i32 ay,
  ...
  rund::kernel::i128 out = cpu::proj::Unit2X_32(ax, ay);
  out += cpu::proj::Unit2Y_32(ax, ay);
  out += cpu::proj::Unit3X_32(ax, ay, az);
  out += cpu::proj::Unit3Y_32(ax, ay, az);
  out += cpu::proj::Unit3Z_32(ax, ay, az);
  out += cpu::proj::Proj2X_32(ax, ay, bx, by);
  out += cpu::proj::Proj2Y_32(ax, ay, bx, by);
  out += cpu::proj::Proj3X_32(ax, ay, az, bx, by, bz);
  out += cpu::proj::Proj3Y_32(ax, ay, az, bx, by, bz);
  out += cpu::proj::Proj3Z_32(ax, ay, az, bx, by, bz);
...
Expected64(const rund::kernel::i64 ax, const rund::kernel::i64 ay,
  ...
  rund::kernel::i128 out = cpu::proj::Unit2X_64(ax, ay);
  out += cpu::proj::Unit2Y_64(ax, ay);
  out += cpu::proj::Unit3X_64(ax, ay, az);
  out += cpu::proj::Unit3Y_64(ax, ay, az);
  out += cpu::proj::Unit3Z_64(ax, ay, az);
  out += cpu::proj::Proj2X_64(ax, ay, bx, by);
  out += cpu::proj::Proj2Y_64(ax, ay, bx, by);
  out += cpu::proj::Proj3X_64(ax, ay, az, bx, by, bz);
  out += cpu::proj::Proj3Y_64(ax, ay, az, bx, by, bz);
  out += cpu::proj::Proj3Z_64(ax, ay, az, bx, by, bz);
...
[[nodiscard]] bool RunsDeterministicProjDslOps() {
  std::array<rund::kernel::i32, 4u> ax32{0, 0x20000000, -0x10000000,
                                         0x30000000};
  std::array<rund::kernel::i32, 4u> ay32{0, 0x10000000, 0x20000000,
                                         -0x10000000};
  std::array<rund::kernel::i32, 4u> az32{0, -0x10000000, 0x10000000,
                                         0x20000000};
  std::array<rund::kernel::i32, 4u> bx32{0, 0x40000000, 0x10000000,
                                         -0x20000000};
  std::array<rund::kernel::i32, 4u> by32{0, 0, -0x10000000, 0x10000000};
  std::array<rund::kernel::i32, 4u> bz32{0, 0x10000000, 0x20000000,
                                         -0x10000000};
  std::array<rund::kernel::i32, 4u> out32{};
  TEST_ASSERT(RunProjOp(ax32, ay32, az32, bx32, by32, bz32, out32));
  ...
  std::array<rund::kernel::i64, 4u> ax64{
      0, 0x2000000000000000ll, -0x1000000000000000ll, 0x3000000000000000ll};
  ...
  std::array<rund::kernel::i64, 4u> az64{
      0, -0x1000000000000000ll, 0x1000000000000000ll, 0x2000000000000000ll};
  std::array<rund::kernel::i64, 4u> bx64{
      0, 0x4000000000000000ll, 0x1000000000000000ll, -0x2000000000000000ll};
  std::array<rund::kernel::i64, 4u> by64{0, 0, -0x1000000000000000ll,
                                         0x1000000000000000ll};
  std::array<rund::kernel::i64, 4u> bz64{
      0, 0x1000000000000000ll, 0x2000000000000000ll, -0x1000000000000000ll};
  std::array<rund::kernel::i64, 4u> out64{};
  TEST_ASSERT(RunProjOp(ax64, ay64, az64, bx64, by64, bz64, out64));
