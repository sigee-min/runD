...
int StartLoopbackClient(const sockaddr_in& address,
...
int StartLoopbackClientWithByte(const sockaddr_in& address,
                                const std::byte byte,
                                ServerSocketCleanup& cleanup) {
  TEST_ASSERT(StartLoopbackClient(address, cleanup) == 0);
