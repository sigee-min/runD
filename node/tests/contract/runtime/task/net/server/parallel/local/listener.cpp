...
int PrepareServerParallelLoopbackListener(
  ...
  TEST_ASSERT(::listen(listener_fd, kServerParallelListenerBacklog) == 0);

  socklen_t length = sizeof(fixture.address);
  TEST_ASSERT(::getsockname(listener_fd,
                            reinterpret_cast<sockaddr *>(&fixture.address),
                            &length) == 0);

  fixture.listener = rund::node::test::net::admit(fixture.listener_cleanup.fd);
  TEST_ASSERT(rund::node::test::net::native(fixture.listener) >= 0);
