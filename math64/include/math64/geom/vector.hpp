...
namespace detail {
inline simd::I64x AbsLane(const simd::I64x value) noexcept {
  const simd::I64x zero = simd::SplatI64(0);
  const simd::I64x negated = simd::Sub(zero, value);
  const simd::I64x wrapped = simd::Select(simd::Lt(value, zero), negated, value);
