...
Math64 SoA APIs are the public prepared range surface for 64-bit component
arrays. They bind existing `x`, `y`, and `z` arrays to vector laws without
object-vector staging or hidden scalar formula loops.
...
`/math64/include/math64/soa/range.hpp` is the single private byte-range
