...
`--bulk`, and `--pipeline` modes are the
deliberate exceptions to the installed-SDK route: they share one incremental
current-source Release build in `.cache/measure/compute/focus` for the
edit/measure loop and are not baseline evidence. This tree is disjoint from
...
measured samples; both modes require graph/output hash parity and zero warm
compile/allocation/transfer counters. The selected accelerator receives fifteen
timed samples in sort and bulk mode; the CPU oracle receives one unless it is
the selected backend. Bulk reports value traffic separately
from logical coefficient reads.
